...
--STORE Add Employees
go
create proc sp_Add_Employee(@manv int, @tennv nvarchar(50),@ngaysinhnv date, @luongnv int,@gioitinh int, @p_id int, @d_id int)
as
insert into EMPLOYEES values (@manv, @tennv, @ngaysinhnv,@gioitinh ,@luongnv , @p_id, @d_id)

--STORE UPDATE EMPLOYEES
go
create proc sp_Update_Employee(@id int, @tennv nvarchar(50),@ngaysinhnv date, @luongnv int,@gioitinh int , @p_id int, @d_id int)
as
update EMPLOYEES
set ID = @id
,NAME = @tennv
...
--STORE ADD POSITON
go
create proc sp_Add_Position(@id int,@name nvarchar(50))
as
BEGIN
INSERT INTO POSITION VALUES(@id,@name)
END

--STORE UPDATE POSITION
go
create proc sp_Update_Position(@id int,@name nvarchar(50))
as
BEGIN
update POSITION
set ID = @id
...
go
create proc sp_Delete_Position(@id int)
as 
BEGIN
...
--STORE ALL POSITION
go
create proc sp_All_Position(@id int)
as
BEGIN
...
go
create proc sp_Add_Department(@id int,@name nvarchar(50))
as
BEGIN
...
go
create proc sp_Delete_Department(@id int)
as
BEGIN
...
go
create proc sp_Update_Department(@id int,@name nvarchar(50))
as
BEGIN
update DEPARTMENT
set 
...
--STORE ALL DEPARTMENT
go
create proc sp_All_Department
as
BEGIN
